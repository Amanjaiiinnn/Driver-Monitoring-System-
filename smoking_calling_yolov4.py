...
class SmokingCallingDetector:
    ...
    def __init__(self, model_path, iou=0.25, conf=0.55):
        ...
        if self._model_file.endswith(".nb"):
            ...
            #print("[INFO] Loading .nb model with HW acceleration enabled")
            ...
            #raise RuntimeError("Only .nb model supported now")
    ...
    def inference(self, input_image, mono):
        ...
        if self.use_nb_model:
            ...
            # -----------------------------
            pred = [
                self.stai_mpu_model.get_output(i)
                for i in range(len(self.output_infos))
            ]
        else:
            # send data
        ...
        if len(self.result) > 0:
            ...
            for i in range(0, num_result):
                ...
                # THEN append extra values
                temp.append(float(self.result[1][i]))  # confidence
                temp.append(int(self.result[2][i]))    # class id
